...
```bash
{
    "packer_template": {
        ...
        "provisioners": [
            ...
            "/provisioners/config-basic-security.json"
        ],
        "post_processors": [
...
##### ./packer-modules/provisioners/config-basic-security.json 
```json
{
    "type": "shell",
    "environment_vars" : [
    ...
    "scripts": [
        "{{user `global_provisioner_path`}}/shell-script/config-basic-security.sh"
    ]
}
```
